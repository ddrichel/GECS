...
|**SINGLEMARKER**|	_\<bool\>_|0|whether single-marker analysis should be performed instead of VB|If SINGLEMARKER=1, then the standrad single marker analysis (SMA) will be performed and all variants will be included in the analysis. For SINGLEMARKER=0, the VB approach will be approach, where threshold of the minor allele frequency of variants included in the analysis nned to be specified.|
